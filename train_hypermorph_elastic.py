...
from test_hypermorph_elastic import test_and_plot
...
class ValidationCallback(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if epoch > 0 and (epoch+1) % val_interval == 0:
            ...
            # val_generator = utils.hyp_generator_elastic_val(1, base_generator, val_dataset.return_mode,
            #                                                 val_lambda=float(LAMBDA_RANGE[1] / 10),
            #                                                 val_mu=float(MU_RANGE[1] / 10))
            val_generator = utils.hyp_generator_elastic_val(1, base_generator, val_dataset.return_mode)
            ...
            while idx < len(val_dataset):
                ...
                dice_sample = eval_metrics.dice(np.floor(label_Y_pred), np.floor(label_Y_numpy))
                dice_sample_per_class = eval_metrics.dice_per_class(np.floor(label_Y_pred), np.floor(label_Y_numpy), classes=val_dataset.classes)
...
def image_loss(y_true, y_pred):
    hyp = (1 - tf.squeeze(model.references.hyper_val[0][0])-tf.squeeze(model.references.hyper_val[0][1])) # mu + lambda <=1
    return hyp * image_loss_func(y_true, y_pred)


def grad_loss(y_true, y_pred):
    lam = tf.squeeze(model.references.hyper_val[0][0])
    mu = tf.squeeze(model.references.hyper_val[0][1])
...
# data stuff
dataset = "LungCTCT"
model_name = "500epochs_ncc_elasticparamssum1"
...
train_generator = utils.hyp_generator_elastic(args.batch_size, base_generator)
val_generator = utils.hyp_generator_elastic(args.batch_size, val_base_generator)
