...
class HyperVxmDenseSpatiallyAdaptive(ne.modelio.LoadableModel):
    """
    Dense HyperMorph network for amortized hyperparameter learning.
    """
    ...
    def __init__(self,
                 ...
                 feat_mult=1,
                 kernel_initializer='he_normal',
                 name='hyper_vxm_dense_spatially_adaptive',
                 **kwargs):
        """ 
        Parameters:
            inshape: Optional input tensor shape (including features). e.g. (192, 192, 192, 2).
        ...
        # build hypernetwork
        # hyp_input = tf.keras.Input(shape=inshape, name='%s_hyp_input' % name) # hyp_tensor
        hyp_input = tf.keras.Input(shape=(*inshape, hyp_feats), name='%s_hyp_input' % name) # hyp_tensor
        # hyp_input = tf.keras.Input(shape=[nb_hyp_params], name='%s_hyp_input' % name)
        # hyp_last = hyp_input
        # for n in range(nb_hyp_layers):
        #     hyp_last = KL.Dense(nb_hyp_units, activation='relu',
        ...
        #treat hyp_input similarly to source and traget images, i.e. encoder part is same
        if nb_unet_features is None:
            nb_unet_features = default_unet_features()

        # build feature list automatically
        if isinstance(nb_unet_features, int):
            if nb_unet_levels is None:
                raise ValueError('must provide unet nb_levels if nb_features is an integer')
            feats = np.round(nb_unet_features * feat_mult ** np.arange(nb_unet_levels)).astype(int)
            nb_unet_features = [
                np.repeat(feats[:-1], nb_unet_conv_per_level),
                np.repeat(np.flip(feats), nb_unet_conv_per_level)
            ]
        elif nb_unet_levels is not None:
            raise ValueError('cannot use nb_levels if nb_features is not an integer')

        ndims = len(hyp_input.get_shape()) - 2
        assert ndims in (1, 2, 3), 'ndims should be one of 1, 2, or 3. found: %d' % ndims
        MaxPooling = getattr(KL, 'MaxPooling%dD' % ndims)

        # extract any surplus (full resolution) decoder convolutions
        enc_nf, _ = nb_unet_features
        nb_dec_convs = len(enc_nf)
        nb_unet_levels = int(nb_dec_convs / nb_unet_conv_per_level) + 1

        if isinstance(max_pool, int):
            max_pool = [max_pool] * nb_unet_levels

        # print("hyp_inputs", hyp_input)
        # print("hyp_feats", hyp_feats)
        # print("feat_mult", feat_mult)
        # print("nb_unet_feats", nb_unet_features)
        # print("nb_unet_lvls", nb_unet_levels)
        # print("nb_unet_conv_per_level", nb_unet_conv_per_level)
        # print("ndims", ndims)
        # print("enc_nf",enc_nf)
        # print("nb_dec_convs ",nb_dec_convs )
        # print("max_pool", max_pool)


        # configure encoder for regularization matrix
        hyp_last = hyp_input
        print(hyp_last)
        for level in range(nb_unet_levels - 1):
            for conv in range(nb_unet_conv_per_level):
                nf = enc_nf[level * nb_unet_conv_per_level + conv]
                layer_name = '%s_enc_conv_%d_%d' % (name, level, conv)
                hyp_last = _conv_block(hyp_last, nf, name=layer_name, do_res=do_res,
                                   kernel_initializer=kernel_initializer)
                # print(hyp_last)


            # temporarily use maxpool since downsampling doesn't exist in keras
            hyp_last = MaxPooling(max_pool[level], name='%s_enc_pooling_%d' % (name, level))(hyp_last)

        hyp_last = KL.Flatten()(hyp_last)
        hyp_last = KL.Dense(nb_hyp_units, activation='relu',
                                name='%s_hyp_dense_%d' % (name, 0))(hyp_last)
        # print("hyp_last",hyp_last)
