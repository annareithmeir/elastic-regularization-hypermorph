...
class NCC:
    ...
    def loss(self, y_true, y_pred, reduce='mean', img_mask=None):
        print("in ncc loss", y_true.shape, y_pred)
        # compute cc
        cc = self.ncc(y_true, y_pred)

        # if img_mask is not None:
        #     print("in ncc(): mask out")
        #     cc= cc * img_mask
...
class NCCSpatiallyAdaptive:
    ...
    def loss(self, y_true, y_pred, weight_tensor, img_mask=None, reduce='mean'):
        print("in nccsa loss", y_true.shape, y_pred.shape, weight_tensor.shape)
        # compute cc
        cc = self.ncc(y_true, y_pred)
        # cc= cc* tf.expand_dims(weight_tensor, axis=-1)
...
class GradSpatiallyAdaptive:
    """
    N-D gradient loss spatially-adaptive.
    loss_mult can be used to scale the loss value - this is recommended if
    the gradient is computed on a downsampled vector field (where loss_mult
    is equal to the downsample factor).
    """
    ...
    def _diffs(self, y):
        ...
        for i in range(ndims):
            ...
            # paddings=[[0,0] for count in range(ndims+2)]
            # paddings[d][1]=1
            # df[i] = tf.pad(df[i], paddings)
        return df

    def loss(self, reg_tensor, y_pred, img_mask=None):
        """
        Reuse the y_true argument for the regularization weight tensor
        returns Tensor of size [bs]
        reg_tensor of size (bs, shape, 1)
        y_pred of size (bs, shape, 1)
        """

        print("in gradlosssa", reg_tensor.shape, y_pred.shape)
        ...
        if img_mask is not None:
            print("in grad loss():  mask out ")
            print(img_mask)
            dif[0] = dif[0] * img_mask[:, :-1, :, :]
            dif[1] = dif[1] * img_mask[:, :, :-1, :]
            dif[2] = dif[2] * img_mask[:, :, :, :-1]

        if len(dif[0].shape)==5 and len(dif)==3:
            dif[0] = dif[0] * reg_tensor[:, :-1, :, :]
            dif[1] = dif[1] * reg_tensor[:, :, :-1, :]
            dif[2] = dif[2] * reg_tensor[:, :, :, :-1]
        else:
